...
import { UserCheck, Lock, ArrowRight, ArrowLeft, AlertCircle } from 'lucide-react';
...
export const AssistantLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await loginAsAssistant(passcode || 'webx2026');
    if (!res.success) {
      setErrorMsg(res.message || 'Invalid Assistant Key. Please try again.');
      setLoading(false);
    }
  };
    ...
    <div className="min-h-[85vh] flex flex-col justify-center items-center px-4 py-8 max-w-md mx-auto w-full">
      ...
      <GlassCard glowAccent="blue" className="w-full shadow-2xl border-slate-200">
        <div className="text-center mb-6">
          ...
          <h2 className="text-xl font-black text-slate-900 tracking-tight">ASSISTANT CONSOLE</h2>
          <p className="text-xs font-semibold text-slate-500 mt-1">
            Authorized scanner login for Web X volunteer assistants
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              <input
                type="password"
                placeholder="Enter Assistant Key (e.g. webx2026)"
                value={passcode}
                onChange={(e) => setPasscode(e.target.value)}
                required
                className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-300 rounded-xl text-sm font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
              />
              <Lock className="w-4 h-4 text-slate-400 absolute left-3.5 top-3.5" />
            </div>
            <p className="text-[11px] text-slate-400 font-medium mt-1">
              Default access key: <code className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-600 font-mono">webx2026</code>
            </p>
          </div>
          ...
          >
            <span>{loading ? 'Authenticating...' : 'Open Assistant Scanner'}</span>
